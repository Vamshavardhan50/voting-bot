...
def fetch_free_proxies():
    ...
    # Countries active during IST daytime (Asia, Oceania)
    # IN, SG, JP, AU, KR, TH, ID, MY, VN, PH, TW, HK
    
    # --- Source 1: ProxyScrape (Country Filtered) ---
    try:
        url = "https://api.proxyscrape.com/v2/?request=displayproxies&protocol=http&timeout=5000&country=IN,SG,JP,AU,KR,TH,ID,MY,VN,PH,TW,HK&ssl=all&anonymity=elite"
        ...
        print(f"[proxy] ProxyScrape targeted fetch failed: {e}")

    # --- Source 2: Geonode (Country Filtered) ---
    try:
        countries = ["IN", "SG", "JP", "AU", "KR", "TH", "ID", "MY", "VN", "PH", "TW", "HK"]
        country_params = "&".join([f"country={c}" for c in countries])
        url = f"https://proxylist.geonode.com/api/proxy-list?limit=500&sort_by=lastChecked&sort_type=desc&protocols=http,https&anonymityLevel=elite&{country_params}"
        req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0"})
        resp = urllib.request.urlopen(req, timeout=10)
        ...
        for p in data.get("data", []):
            ...
            port = p.get("port")
            if ip and port:
                raw_proxies.append({"server": f"http://{ip}:{port}"})
    except Exception as e:
        print(f"[proxy] Geonode targeted fetch failed: {e}")
...
def next_proxy():
    """Get a fresh, unused proxy. Refetches from APIs if the pool is empty."""
    global PROXY_POOL
    ...
    if not PROXY_POOL:
        print("\n[proxy] Proxy pool empty! Fetching a fresh batch...")
        PROXY_POOL = fetch_free_proxies()
        
    if not PROXY_POOL:
        print("\n[proxy] Warning: Failed to fetch new proxies. Defaulting to DIRECT.")
        return None
        
    # Pop the proxy so it is never reused
