...
# YOUR WEBSITE URL
# -------------------------
# URL = "https://app.hidevs.xyz/vote/sentrix-ai-d9zh"4
...
def fetch_free_proxies():
    ...
    # --- Source 2: geonode ---
    try:
        # Simplified URL to avoid 401, fetching a large batch
    ...
    try:
        ...
        for line in raw.strip().splitlines():
            line = line.strip()
            # Ensure it looks like a valid ip:port format and is HTTP proxy
            if ":" in line:
                raw_proxies.append({"server": f"http://{line}"})
    except Exception as e:
        print(f"[proxy] redscrape failed: {e}")
...
async def run_voting():
    ...
    while True:
        ...
        if USE_PROXIES:
            ...
            print("[proxy] USE_PROXIES=False. Loading from proxies.txt...")
            PROXY_POOL = []
            raw_proxies = []
            import os
            if os.path.exists("proxies.txt"):
                    ...
                    for line in f:
                        ...
                        if line:
                            if not line.startswith("http"):
                                line = f"http://{line}"
                            raw_proxies.append({"server": line})
                print(f"[proxy] Loaded {len(raw_proxies)} proxies from proxies.txt. Testing for live ones...")
                ...
                with ThreadPoolExecutor(max_workers=50) as executor:
                    ...
                    if tqdm:
                        ...
                        for future in as_completed(future_to_proxy):
                            res = future.result()
                            if res:
                                PROXY_POOL.append(res)
                                pbar.set_postfix_str(str(len(PROXY_POOL)))
                            pbar.update(1)
                        pbar.close()
                    else:
                        results = executor.map(check_proxy, raw_proxies)
                ...
                print(f"[proxy] Kept {len(PROXY_POOL)} LIVE proxies from proxies.txt")
            else:
                print("[proxy] ⚠️ proxies.txt not found. Running without proxies.")
